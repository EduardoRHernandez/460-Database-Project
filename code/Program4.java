...
public class Program4 {
	...
	private static final String QUERY2_STRING = "SELECT * FROM ( "
			...
			+ "FROM Rental r JOIN Equipment e ON e.RID = r.RID " + ") WHERE REF_ID = ? " + "ORDER BY DETAIL2";
	 ...
	 * 
	 *---------------------------------------------------------------------*/
	private static void logEquipmentChange(Connection conn, int eid, String oldType, String newType, String oldSize,
			String newSize, String oldStatus, String newStatus) throws SQLException {
	...
	private static void updateEquipmentItem(Connection conn, int eid) throws SQLException {
		...
		if (rowsAffected > 0) {
			logEquipmentChange(conn, eid, oldType, newType, oldSize, newSize, oldStatus, newStatus);
			conn.commit();
	...
	private static void archiveEquipmentItem(Connection conn, int eid) throws SQLException {
		...
		try (PreparedStatement archiveStmt = conn.prepareStatement(archiveSQL)) {
			...
			System.out.println(
					rowsAffected > 0 ? "Equipment item archived successfully." : "Failed to archive equipment item.");
			conn.commit();
			conn.setAutoCommit(true);
		}
	...
	private static void addMember(Connection conn, Scanner input) throws SQLException {
		String add_member_string = "INSERT INTO member ( " + "FIRSTNAME, LASTNAME, PHONE, EMAIL, DOB, "
	...
	private static void deleteMember(Connection conn, Scanner input) throws SQLException {
		...
		if (!memberExists(conn, memberId)) {
			...
			if (hasAny(conn, "SELECT passid FROM skipass WHERE memberId = " + memberId, // + " AND EXPIRATIONDATE >=
																						// SYSDATE",
					"PASSID",
				...
				// System.out.println("Active ski pass(es) present. Please delete or use them
			...
			if (hasAny(conn,
				...
				// System.out.println(
				// "Active lesson purchase(s) present. Please delete or use them before deleting
				// account");
				unableToReturn = true;
			}
			...
			if (hasAny(conn,
				...
				// System.out.println("Active rental(s) present. Please delete or use them
				// before deleting account");
				unableToReturn = true;
			}
				...
				while (true) {
					...
					case "2":
						...
						try (PreparedStatement stmt = conn.prepareStatement(QUERY2_STRING)) {
							...
							try (ResultSet rs = stmt.executeQuery()) {
								while (rs.next()) {
									System.out.printf("[%s] ID: %s | %s | %s | %s%n", rs.getString("SECTION"),
											rs.getString("REF_ID"), rs.getString("DETAIL1"), rs.getString("DETAIL2"),
					...
					case "3":
						try (PreparedStatement stmt = conn.prepareStatement(QUERY3_STRING);
							...
							while (rs.next()) {
								System.out.printf("Trail: %s, Category: %s, Lift: %s%n", rs.getString("trail_name"),
										rs.getString("category"), rs.getString("lift_name"));
							}
						}
						break;
					case "4":
